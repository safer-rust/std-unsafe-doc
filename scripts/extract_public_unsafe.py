...
def write_html(all_items, output_path, rustc_version):
    ...
    lines = [
        ...
        "        if (cb && cb.checked) ids.push(r.dataset.id);",
        "      });",
        "      return ids;",
        "    }",
        "",
        ...
        "      if (checked.length > 0) params.set('c', checked.join(','));",
        ...
        "      var checkedIds = params.get('c');",
        "      if (checkedIds) {",
        "        var set = {};",
        "        checkedIds.split(',').forEach(function (id) { set[id] = true; });",
        "        getRows().forEach(function (r) {",
        "          var cb = r.querySelector('.confirm-cb');",
        "          if (cb && set[r.dataset.id]) {",
        "            cb.checked = true;",
        ...
        "        getRows().forEach(function (r) {",
        "          var cb = r.querySelector('.confirm-cb');",
        "          if (cb && r.dataset.id in state && state[r.dataset.id]) {",
        "            cb.checked = true;",
        "            r.classList.add('row-confirmed');",
        "          }",
        "        });",
